...
import { lessonProgressSchema, } from "./roadmap.schemas.js";
 ...
 */
function readAiRoadmapPayload(value) {
    if (!isRecord(value)) {
        return null;
    }
    const roadmapValue = value["roadmap"];
    if (!isRecord(roadmapValue)) {
        return null;
    }
    const modulesValue = roadmapValue["modules"];
    if (!Array.isArray(modulesValue) ||
        modulesValue.length === 0) {
        return null;
    }
    const modules = [];
    for (const rawModule of modulesValue) {
        if (!isRecord(rawModule)) {
            return null;
        }
        const moduleId = readString(rawModule["moduleId"]);
        const moduleName = readString(rawModule["name"]);
        ...
        for (const rawLesson of rawLessons) {
            ...
            const lessonId = readString(rawLesson["lessonId"]);
            const lessonName = readString(rawLesson["name"]);
            if (!lessonId ||
                !lessonName) {
                return null;
            }
            const rawResources = rawLesson["resources"];
            const resources = Array.isArray(rawResources)
                ? rawResources
                    .filter(isRecord)
                    .map(resource => ({
                    resourceId: readString(resource["resourceId"]),
                    label: readString(resource["label"]),
                    title: readString(resource["title"]),
                    url: readString(resource["url"]) ?? "",
                }))
                    .filter(resource => Boolean(resource.url))
                : [];
            const rawChildren = rawLesson["children"];
            const children = Array.isArray(rawChildren)
                ? rawChildren.filter((child) => typeof child ===
                    "string")
                : [];
            lessons.push({
                lessonId,
                name: lessonName,
                description: readString(rawLesson["description"]),
                estimatedHours: readPositiveInteger(rawLesson["estimatedHours"]),
                xpReward: readPositiveInteger(rawLesson["xpReward"]),
                children,
                resources,
            });
        }
        modules.push({
            moduleId,
            name: moduleName,
            description: readString(rawModule["description"]),
            lessons,
        });
    }
    return {
        roadmap: {
            title: readString(roadmapValue["title"]),
            summary: readString(roadmapValue["summary"]),
            modules,
        },
        message: readString(value["message"]),
    };
}
function readAiDraftMetadata(value) {
    if (!isRecord(value)) {
        return {};
    }
    return {
 ...
 */
function createAiLessonRows(payload) {
    const rows = [];
    let globalSortOrder = 0;
    payload.roadmap.modules.forEach((module, moduleIndex) => {
        module.lessons.forEach((lesson, lessonIndex) => {
            globalSortOrder +=
                1;
            const estimatedHours = lesson.estimatedHours ??
                null;
            const calculatedXp = estimatedHours
                ? Math.max(10, Math.min(200, estimatedHours *
                    10))
                : 10;
            rows.push({
                /*
                ...
                estimatedHours,
                resources: toInputJson(lesson.resources ??
                    []),
                publicLessonId: lesson.lessonId,
                 ...
                 * as active.
                 */
                status: globalSortOrder === 1
                    ? "IN_PROGRESS"
                    : "NOT_STARTED",
                children: toInputJson(lesson.children ??
                    []),
                completedAt: null,
            });
        });
 ...
 */
function createFallbackLessonRows(generated) {
    const publicLessonIds = generated.milestones.map((_milestone, index) => `fallback_m1_l${index + 1}`);
    return generated.milestones.map((milestone, index) => {
        const nextLessonId = publicLessonIds[index + 1];
        const estimatedHours = milestone.estimatedHours;
        const calculatedXp = estimatedHours
            ? Math.max(10, Math.min(200, estimatedHours *
                10))
            : 10;
        return {
            ...
            estimatedHours,
            resources: toInputJson(milestone.resources),
            publicLessonId: publicLessonIds[index],
            moduleId: "fallback_module_1",
            moduleName: generated.title,
            moduleDescription: generated.description,
            moduleOrder: 1,
            lessonOrder: index + 1,
            ...
            status: index === 0
                ? "IN_PROGRESS"
                : "NOT_STARTED",
            children: toInputJson(nextLessonId
                ? [nextLessonId]
                : []),
            completedAt: null,
        };
...
roadmapRouter.get("/", asyncHandler(async (request, response) => {
    ...
    response.json({
        roadmaps: roadmaps.map(roadmap => ({
            ...
            milestoneCount: roadmap
                .milestones
                .length,
            completedCount: roadmap
                .milestones
...
roadmapRouter.post("/from-assessment/:assessmentId", asyncHandler(async (request, response) => {
    ...
    const existing = await prisma.savedRoadmap.findUnique({
        where: {
            assessmentId,
        },
    });
     ...
     */
    const aiGeneration = assessment.aiGenerations.find(generation => generation.kind ===
        "ROADMAP" &&
        generation.finalPayload);
    const aiPayload = aiGeneration
        ? readAiRoadmapPayload(aiGeneration
            .finalPayload)
        : null;
    let title;
    let summary;
    ...
    if (aiPayload &&
        aiPayload.roadmap
            .modules.length > 0) {
        const draftMetadata = readAiDraftMetadata(aiGeneration
            ?.draftPayload);
        title =
            aiPayload.roadmap
                .title ??
        ...
        source =
            "AI";
        roadmapJson =
            toInputJson(aiPayload);
        lessonRows =
            createAiLessonRows(aiPayload);
    }
    else {
        /*
        ...
        source =
            "RULE_BASED_FALLBACK";
        roadmapJson =
            toInputJson(generated);
        lessonRows =
            createFallbackLessonRows(generated);
        if (aiGeneration) {
            console.warn(`Assessment ${assessmentId} had an invalid AI finalPayload. The rule-based fallback was saved instead.`);
        }
    }
    if (lessonRows.length === 0) {
        throw new HttpError(409, "ROADMAP_HAS_NO_LESSONS", "The generated roadmap does not contain any lessons.");
    }
...
roadmapRouter.patch("/:roadmapId/milestones/:milestoneId", asyncHandler(async (request, response) => {
    ...
    if (!milestone) {
        throw new HttpError(404, "MILESTONE_NOT_FOUND", "The roadmap step was not found.");
    }
    const milestones = await prisma.$transaction(async (transaction) => {
        /*
        ...
        if (input.status ===
            "COMPLETED") {
            const activeLesson = await transaction.savedMilestone.findFirst({
                where: {
                    savedRoadmapId: roadmapId,
                    status: "IN_PROGRESS",
                },
            ...
            if (!activeLesson) {
                const nextLesson = await transaction.savedMilestone.findFirst({
                    where: {
                        savedRoadmapId: roadmapId,
                        sortOrder: {
    ...
    const updatedMilestone = milestones.find(item => item.id ===
        milestone.id);
    await captureEvent({
        userId,
        sessionId: visitorId(request),
...
roadmapRouter.delete("/:roadmapId", asyncHandler(async (request, response) => {
    const userId = currentUserId(request);
    const roadmapId = routeParam(request, "roadmapId");
    const deleted = await prisma.savedRoadmap.deleteMany({
        where: {
            id: roadmapId,
            userId,
        },
