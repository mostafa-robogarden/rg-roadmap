...
async function request(jar, path, init = {}) {
    ...
    if (xsrf && !["GET", "HEAD", "OPTIONS"].includes(init.method ?? "GET"))
...
const catalog = await request(guest, "/api/tracks");
if (!catalog.tracks.length)
    throw new Error("No tracks returned.");
const started = await request(guest, "/api/assessments", {
    method: "POST",
    body: JSON.stringify({ trackSlug: catalog.tracks[0].slug }),
});
const assessment = await request(guest, `/api/assessments/${started.assessmentId}`);
...
const saved = await request(guest, `/api/roadmaps/from-assessment/${started.assessmentId}`, { method: "POST", body: "{}" });
const roadmap = await request(guest, `/api/roadmaps/${saved.roadmapId}`);
