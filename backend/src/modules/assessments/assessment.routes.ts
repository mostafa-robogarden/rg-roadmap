...
assessmentRouter.post(
  ...
  asyncHandler(
    async (request, response) => {
      ...
      const track =
        await findPublishedTrackBySlug(
          input.trackSlug,
        );
      ...
      if (!track) {
        throw new HttpError(
          404,
          "TRACK_NOT_FOUND",
          "The roadmap track was not found.",
        );
      }
       ...
       */
      const questionnaire =
        await generateQuestionnaireDraft({
...
assessmentRouter.get(
  ...
  asyncHandler(
    async (request, response) => {
      ...
      const assessment =
        await findAssessmentWithContent(id);
      ...
      if (
        ...
        throw new HttpError(
          409,
          "QUESTIONNAIRE_NOT_GENERATED",
          "This assessment does not have a generated questionnaire.",
        );
      }
      ...
      response.json({
        assessment: {
          ...
          learnerGoal:
            assessment.learnerGoal,

          weeklyHours:
            assessment.weeklyHours,

          targetMonths:
            assessment.targetMonths,
          ...
          answers:
            assessment.generatedResponses.map(
              answer => ({
                questionId:
                  answer.questionId,
        ...
        questions:
          assessment.generatedQuestions.map(
            question => ({
              id:
                question.id,
    ...
    async (request, response) => {
      ...
      if (questions.length === 0) {
        throw new HttpError(
          409,
          "QUESTIONNAIRE_NOT_GENERATED",
          "This assessment does not have generated questions.",
        );
      }
    ...
    async (request, response) => {
      ...
      if (
        ...
        throw new HttpError(
          403,
          "ASSESSMENT_FORBIDDEN",
          "You cannot access this roadmap.",
        );
      }
