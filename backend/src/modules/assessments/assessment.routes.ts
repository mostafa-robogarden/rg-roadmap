...
function ownsAssessment(
  assessment: { userId: string | null; sessionId: string },
  requestUserId: string | undefined,
  requestVisitorId: string,
): boolean {
  return Boolean(
    (assessment.userId && requestUserId && assessment.userId === requestUserId) ||
...
assessmentRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (!track) throw new HttpError(404, "TRACK_NOT_FOUND", "The roadmap track was not found.");
...
assessmentRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    let totalScore = 0;
    let maximumScore = 0;
    const answerRows: Array<{ assessmentId: string; questionId: string; optionId: string }> = [];
